...
__export(index_exports, {
  ...
  Lexer: () => Lexer,
  LocalAIPlugin: () => LocalAIPlugin,
  OpenAIPlugin: () => OpenAIPlugin,
  PluginManager: () => PluginManager,
  PromptEngine: () => PromptEngine,
  SemanticAnalyzer: () => SemanticAnalyzer,
  default: () => PromptEngine,
  usePromptEngine: () => usePromptEngine
});
...
var ComponentGraph = class {
  ...
  buildSearchIndex() {
    ...
    this.searchIndex = new import_fuse.default(searchableItems, {
...
var PluginManager = class extends import_eventemitter3.EventEmitter {
  constructor() {
    super();
...
// src/core/Engine.ts
var PromptEngine = class extends import_eventemitter32.EventEmitter {
  constructor(config = {}) {
    super();
    // State
    this.initialized = false;
...
function usePromptEngine(schema, config) {
  ...
  const generate = (0, import_react.useCallback)(async (prompt) => {
    if (!engine) {
      throw new Error("Engine not initialized");
    }
    return engine.process(prompt);
  }, [engine]);
  const learn = (0, import_react.useCallback)(async (prompt, correction, expected) => {
    if (!engine) return;
    return engine.learn(prompt, correction, expected);
  }, [engine]);
  const registerPlugin = (0, import_react.useCallback)(async (name, plugin) => {
    if (!engine) return;
    return engine.plugins.register(name, plugin);
  }, [engine]);
  const setActivePlugin = (0, import_react.useCallback)((name) => {
    if (!engine) return;
